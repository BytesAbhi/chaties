...
@section('content')
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($participant->certificate_unlocked)
                                    <a href="{{ route('certificates.download', $participant) }}"
                                        class="btn btn-sm btn-success">Download e-Certificate</a>
                                @endif
                                @if ($participant->certificate_option === 'combo')
                                    <a href="{{ route('certificates.order', $participant) }}"
                                        class="btn btn-sm btn-info">Order Printed</a>
                                @endif
                            </td>
                            <td>
    ...
    <script>
        ...
        navigator.geolocation.getCurrentPosition(position => {
            document.querySelectorAll('[yourself(' [id ^= "latitude-"]
                ').forEach(input => {
